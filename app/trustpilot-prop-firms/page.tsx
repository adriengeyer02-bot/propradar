...
export const metadata: Metadata = {
  ...
  description:
    'PropRadar sépare la note Trustpilot brute de sa fiabilité estimée : volume, alertes avis, incidents payout et cohérence communautaire.',
  alternates: {
    ...
    <main className="container guide-page">
      <section className="guide-hero">
        <div>
          ...
          <p className="lead">
            Une prop firm peut afficher une bonne note publique tout en ayant des signaux à pondérer : avis invités,
            historique court, plaintes payout, changements de règles ou incohérence avec les retours traders.
          </p>
        ...
        <div className="guide-proof-panel">
          ...
          <div><strong>{watchList.length}</strong><span>dossiers à pondérer</span></div>
          <div><strong>{propFirms.filter((firm) => firm.reviewSignals.trustpilotReliability === 'Forte').length}</strong><span>fiabilité forte</span></div>
        </div>
      </section>
      ...
      <section className="section guide-card-grid">
        <article className="panel">
          ...
          <ul className="risk-list">
            <li>Volume et ancienneté des avis, pas seulement la moyenne.</li>
            <li>Alertes d'avis marketing, invités ou trop récents.</li>
            <li>Cohérence avec Reddit, forums et incidents payout.</li>
            <li>Sources officielles et changements de règles récents.</li>
          </ul>
        </article>
        <article className="panel">
          <div className="eyebrow">Règle simple</div>
          <h2>Une note élevée doit être vérifiée.</h2>
          <p className="lead">
            Trustpilot est un signal utile, mais PropRadar le traite comme une pièce du dossier, pas comme une preuve
            finale de fiabilité.
          </p>
        </article>
      </section>
      ...
      <section className="section">
        <div className="section-heading">
          <div>
            ...
            <h2>Firms où la note publique demande du contexte</h2>
          </div>
        </div>
        <div className="guide-ranking-list">
          {watchList.map((firm) => (
            <Link href={`/firm/${firm.slug}`} className="guide-ranking-row trust-row" key={firm.slug}>
              ...
              <div><span>Note brute</span><strong>{firm.trustpilotRating ? `${firm.trustpilotRating}/5` : 'À vérifier'}</strong></div>
              ...
              <div><span>Fiabilité</span><strong className={`badge ${reviewReliabilityClass(firm.reviewSignals.trustpilotReliability)}`}>{firm.reviewSignals.trustpilotReliability}</strong></div>
