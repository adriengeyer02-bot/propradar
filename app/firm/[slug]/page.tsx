...
function scoreProofRows(firm: PropFirm, kind: 'reddit' | 'x' | 'trustpilot' | 'payout' | 'manipulation'): ScoreProofRow[] {
  if (kind === 'reddit') {
    return [
      { label: 'Source', value: firm.reviewSignals.redditSource === 'Estimation PropRadar' ? 'Score brut PropRadar, revue manuelle à compléter' : firm.reviewSignals.redditSource ?? 'À documenter' },
      { label: 'Volume brut', value: `${firm.reviewSignals.redditSampleSize ?? 0} avis / mention(s)` },
  ...
  if (kind === 'x') {
    return [
      { label: 'Source', value: firm.reviewSignals.xSource === 'Estimation PropRadar' ? 'Score brut PropRadar, revue manuelle à compléter' : firm.reviewSignals.xSource ?? 'À documenter' },
      { label: 'Volume brut', value: `${firm.reviewSignals.xSampleSize ?? 0} avis / mention(s)` },
      { label: 'Confiance', value: firm.reviewSignals.xConfidence ?? 'Faible' },
    ];
  }
  ...
  if (kind === 'trustpilot') {
    ...
    return [
      ...
      { label: 'Source', value: trustpilotUrl ? 'Page Trustpilot listée' : 'Lien ou revue à compléter', href: trustpilotUrl },
  ...
  if (kind === 'payout') {
    ...
    return [
      { label: 'Preuve visible', value: firm.payoutProof && payoutUrl ? 'Page payout officielle listée' : firm.payoutProof ? 'Signal suivi, preuve indépendante à publier' : 'Insuffisante', href: firm.payoutProof ? payoutUrl : undefined },
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    ...
    description: `${firm.name} sur PropRadar : score ${firm.score}/100, risque payout ${firm.reviewSignals.payoutRisk}, fiabilité Trustpilot ${firm.reviewSignals.trustpilotReliability}. ${firm.verdict}`,
    alternates: {
    ...
    <main>
        ...
        {isBlueGuardian ? (
          <div className="panel blue-guardian-affiliate-panel">
            <div className="blue-guardian-affiliate-copy">
              <div className="eyebrow">Partenaire Blue Guardian</div>
              <h2>Code Blue Guardian PropRadar : {PROPRADAR_PROMO_CODE}</h2>
              <p className="muted">
                Utilise le lien affilié PropRadar puis vérifie le code au checkout. Blue Guardian reste évaluée séparément :
                score, risque payout, signaux Reddit/X, Trustpilot et règles de trading ne sont pas modifiés par l'affiliation.
              </p>
              <div className="promo-proof-row">
                <span>Code {PROPRADAR_PROMO_CODE}</span>
                <span>Lien affilié déclaré</span>
                <span>À vérifier au checkout</span>
              </div>
              <a href={BLUE_GUARDIAN_AFFILIATE_URL} target="_blank" rel="noreferrer sponsored" className="btn btn-primary">
                Ouvrir Blue Guardian avec le code
              </a>
            </div>
            <div className="blue-guardian-offer-card">
              <span>Offre PropRadar</span>
              <strong>{PROPRADAR_PROMO_CODE}</strong>
              <small>Forex, Futures et programmes Blue Guardian selon disponibilité au checkout.</small>
            </div>
          </div>
        ) : null}
        ...
        {isThe5ers ? (
          <div className="panel the5ers-affiliate-panel">
            <div className="the5ers-affiliate-copy">
              <div className="eyebrow">Partenaire The5ers</div>
              <h2>Coupon The5ers PropRadar : {THE5ERS_PROMO_CODE}</h2>
              <p className="muted">
                Le lien est affilié et le coupon est à vérifier au checkout. Le score PropRadar reste indépendant :
                règles, payout, Trustpilot, Reddit/X et niveau de preuve sont lus séparément de la relation commerciale.
              </p>
              <div className="promo-proof-row">
                <span>Coupon {THE5ERS_PROMO_CODE}</span>
                <span>Lien affilié déclaré</span>
                <span>Score indépendant</span>
              </div>
              <a href={THE5ERS_AFFILIATE_URL} target="_blank" rel="noreferrer sponsored" className="btn btn-primary">
                Ouvrir The5ers avec le coupon
              </a>
            </div>
      ...
      <section className="container section" id="audit">
        ...
        <div className="grid-3">
          <div className="card">
            <div className="muted">Reddit score brut</div>
            ...
            {firm.reviewSignals.redditSource === 'Estimation PropRadar' ? <span className="badge badge-amber">Score brut manuel</span> : null}
          ...
          <div className="card">
            <div className="muted">X/Twitter score brut</div>
            <div className="score">{firm.reviewSignals.xScore}/100</div>
            <span className={`badge ${scoreClass(firm.reviewSignals.xScore)}`}>{firm.reviewSignals.xSignal}</span>
            {firm.reviewSignals.xSource === 'Estimation PropRadar' ? <span className="badge badge-amber">Score brut manuel</span> : null}
            <div className="sentiment-breakdown">
              <span><strong>{firm.reviewSignals.xPositiveMentions ?? 0}</strong> positifs</span>
              <span><strong>{firm.reviewSignals.xNegativeMentions ?? 0}</strong> négatifs</span>
              <span><strong>{firm.reviewSignals.xNeutralMentions ?? 0}</strong> neutres</span>
            </div>
            <p className="muted" style={{ marginTop: 12, marginBottom: 0 }}>{firm.reviewSignals.xMentions}</p>
            <div className="score-proof">
        ...
        <div className="panel manual-review-panel">
          <div className="manual-review-header">
            <div>
              <div className="eyebrow">Revue manuelle Reddit/X</div>
              <h3>Collecte à sourcer</h3>
            </div>
            <span className={`badge ${manualCoverageClass(manualReview.coverage)}`}>{manualReview.coverage}</span>
          </div>
          <p className="muted">{manualReview.summary}</p>
        ...
        <div className="signal-explain-grid">
          ...
          <div className="signal-explain-card">
            ...
            <ul className="risk-list">
              {(firm.reviewSignals.xFlags ?? []).map((flag) => <li key={flag}>{flag}</li>)}
            </ul>
          </div>
          <div className="signal-explain-card">
      ...
      <section className="container section" id="guides">
        <div className="section-heading">
          <div>
            ...
            <h2>Guides PropRadar liés à {firm.name}</h2>
          </div>
  ...
  return (
    <article className="manual-review-card">
      <div className="manual-review-card-top">
        <strong>{title}</strong>
        <span className={`badge ${manualCoverageClass(channel.coverage)}`}>{channel.coverage}</span>
      </div>
      {typeof channel.sampleSize === 'number' ? (
        <div className="sentiment-breakdown">
          <span><strong>{channel.positive ?? 0}</strong> positifs</span>
          <span><strong>{channel.negative ?? 0}</strong> négatifs</span>
          <span><strong>{channel.neutral ?? 0}</strong> neutres</span>
