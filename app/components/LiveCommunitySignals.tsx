  ...
  useEffect(() => {
    ...
    async function loadSignals() {
      try {
        ...
        if (!response.ok) {
          throw new Error(`API signals ${response.status}`);
        }

        setData((await response.json()) as SignalsResponse);
      } catch (loadError) {
        if (controller.signal.aborted) return;
        setError(loadError instanceof Error ? loadError.message : 'Signal live indisponible');
      }
    }
  ...
  return (
    <div className="panel live-signal-panel">
      <div className="live-signal-header">
        <div>
          <div className="eyebrow">API live si clés configurées</div>
          <h3>Reddit et X/Twitter</h3>
        </div>
        <span className="badge badge-blue">{data ? 'Synchronisé' : error ? 'Fallback' : 'Connexion'}</span>
      </div>

      {error ? (
        <p className="muted">Impossible de joindre la route live pour le moment : {error}. Les scores estimés restent visibles plus haut.</p>
      ) : null}
...
function LiveSignalSkeleton({ label }: { label: string }) {
  return (
    <article className="live-signal-card live-signal-loading">
      ...
      <p className="muted">Connexion à la route API...</p>
    </article>
  );
}

function statusLabel(status: LiveSignal['status'], configured: boolean) {
  if (status === 'live') return 'API active';
  if (!configured) return 'Clé API à ajouter';
  return 'Erreur API';
}
