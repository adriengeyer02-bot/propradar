  ...
  useEffect(() => {
    ...
    async function loadSignals() {
      try {
        ...
        setError(loadError instanceof Error ? loadError.message : 'Signal unavailable');
      }
    }
  ...
  return (
    <div className="panel live-signal-panel">
      <div className="live-signal-header">
        <div>
          <div className="eyebrow">Raw Community Score</div>
          <h3>Positive and negative reviews</h3>
        </div>
        <span className="badge badge-blue">{data ? 'Loaded' : error ? 'Raw score' : 'Loading'}</span>
      </div>

      {error ? (
        <p className="muted">Unable to refresh these scores right now: {error}. Raw scores remain visible above.</p>
      ) : null}
  ...
  return (
    <article className={`live-signal-card live-signal-${signal.status}`}>
      ...
      <div className="live-signal-score">
        ...
        <strong>{toEnglishText(signal.signal)}</strong>
      </div>
      <div className="sentiment-breakdown">
        <span><strong>{signal.positive}</strong> positive</span>
        <span><strong>{signal.negative}</strong> negative</span>
        <span><strong>{signal.neutral}</strong> neutral</span>
      </div>
      <div className="score-proof">
        ...
        <div><span>Confidence</span><strong>{toEnglishText(signal.confidence)}</strong></div>
      </div>
...
function LiveSignalSkeleton({ label }: { label: string }) {
  return (
    <article className="live-signal-card live-signal-loading">
      ...
      <p className="muted">Loading raw scores...</p>
    </article>
  );
}

function statusLabel(status: LiveSignal['status'], configured: boolean) {
  if (status === 'live') return 'Review loaded';
  if (!configured) return 'Raw score';
  return 'To verify';
}
