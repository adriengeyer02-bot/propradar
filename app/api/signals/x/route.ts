...
import { fetchXSignals, parseSignalLimit } from '../../../lib/communityApis';
...
export async function GET(request: NextRequest) {
  ...
  if (!firm) {
    return NextResponse.json(
      {
        error: 'Ajoute ?slug=ftmo ou ?firm=FTMO pour interroger le signal X/Twitter.',
      },
      { status: 400 },
    );
  }
  ...
  const x = await fetchXSignals(firm, limit);
