        ...
        <div className="table-actions">
          <div>
            ...
            <span>
              ...
              Fais glisser horizontalement pour voir tous les signaux.
            </span>
          </div>
          <div className="table-scroll-buttons" aria-label="Faire défiler le tableau">
            <button type="button" className="table-reset-button" onClick={resetFilters}>Réinitialiser</button>
            <button type="button" className="table-arrow-button" onClick={() => scrollTable('left')} aria-label="Défiler vers la gauche">
              <span aria-hidden="true">&larr;</span>
            </button>
            <button type="button" className="table-arrow-button" onClick={() => scrollTable('right')} aria-label="Défiler vers la droite">
              ...
              <tbody>
                {filteredFirms.length === 0 ? (
                  ...
                  <tr key={firm.slug} className={index === 0 ? 'table-row-featured' : undefined}>
                    ...
                    <td><span className={`badge ${manipulationRiskClass(firm.reviewSignals.manipulationRisk ?? 'Moyen')}`}>{firm.reviewSignals.manipulationRisk ?? 'Moyen'}</span></td>
                    <td>
                      <span className={`badge ${relationshipClass(firm.commercialRelationship)}`}>
                        {firm.commercialRelationship === 'Affiliation transparente' ? 'Affilié' : 'Aucun'}
                      </span>
                    </td>
                    <td className="product-cell">{firm.products[0]?.name ?? 'Aucun produit actif'}</td>
