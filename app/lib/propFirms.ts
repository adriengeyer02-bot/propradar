...
function buildReviewSignals(data: Omit<PropFirm, 'reviewSignals'>): ReviewSignals {
  ...
  return {
    ...
    trustpilotFlags: data.trustpilotRating
      ? ['Note utile mais non suffisante', 'Pondération par incidents et preuves externes']
      : ['Pas de note robuste', 'Ne pas utiliser Trustpilot seul'],
    ...
    confidenceDrivers: [
      data.payoutProof ? 'Preuves de payout présentes' : 'Preuves de payout insuffisantes',
...
function universeFirm(data: UniverseFirmInput): PropFirm {
  ...
  const logoDomain = data.logoDomain ?? domainFromWebsite(data.website);
  ...
  return firm({
    ...
    sources: [{ label: `Site officiel ${data.name}`, url: data.website }],
  });
}
...
const corePropFirms: PropFirm[] = [
  firm({
    ...
    reviewSignals: {
      ...
      trustpilotFlags: ['Volume d’avis significatif', 'Signal cohérent avec l’ancienneté', 'Risque d’avis marketing limité mais pas nul'],
...
const universePropFirms: PropFirm[] = [
  ...
  universeFirm({ id: 61, slug: 'funderpro', name: 'FunderPro', website: 'https://funderpro.com/', status: 'Active', score: 73, founded: 2023, headquarters: 'Europe', priceFrom: 69, payoutProof: true }),
