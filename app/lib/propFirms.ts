...
function buildReviewSignals(data: Omit<PropFirm, 'reviewSignals'>): ReviewSignals {
  ...
  return {
    ...
    xMentions:
      data.status === 'Fermée'
        ? 'archives X/Twitter à traiter comme signaux historiques, pas comme preuve actuelle'
        : data.incidents >= 3
          ? 'plaintes publiques et threads X/Twitter à recouper avec les payouts récents'
          : 'veille X/Twitter estimée : tweets publics, réponses et mentions à confirmer par API ou revue manuelle',
