...
function buildReviewSignals(data: Omit<PropFirm, 'reviewSignals'>): ReviewSignals {
  ...
  return {
    ...
    xMentions:
      data.status === 'Fermée'
        ? 'score brut PropRadar : archives X/Twitter à traiter comme signaux historiques, pas comme preuve actuelle'
        : data.incidents >= 3
          ? 'score brut PropRadar : plaintes publiques et threads X/Twitter à recouper avec les payouts récents'
          : 'score brut X/Twitter : avis positifs, négatifs et neutres à relire manuellement avant décision',
    ...
    products: isClosed
      ...
      : [
          product({
            ...
            description: data.productDescription ?? `Programme ${market} ajouté à la couverture universe. Conditions à confirmer sur la source officielle.`,
            ...
            profitTarget: data.profitTarget ?? 'variable selon programme',
            ...
            platforms: data.platforms ?? (isFutures ? ['NinjaTrader', 'Tradovate', 'Rithmic', 'À vérifier'] : ['MT4', 'MT5', 'cTrader', 'À vérifier']),
            tradableAssets: data.tradableAssets ?? (isFutures ? ['Futures'] : market === 'Crypto' ? ['Crypto'] : market === 'Actions' ? ['Actions'] : ['Forex', 'Indices', 'Métaux', 'Crypto']),
...
const corePropFirms: PropFirm[] = [
  ...
  firm({
    ...
    commercialRelationship: 'Affiliation transparente',
    commercialNote: 'PropRadar peut recevoir une commission via le lien The5ers. Le coupon 1EIJ6PO est affiché comme avantage promotionnel à vérifier au checkout, sans impact sur le score.',
    products: [
      product({
  ...
  firm({
    ...
    products: [
      product({
        ...
        hasConsistencyRule: true,
        consistencyRule: 'Règles exactes et add-ons à confirmer au checkout FXIFY.',
      ...
      product({
        ...
        minTradingDays: '5 jours minimum avant premier payout selon page officielle',
        hasConsistencyRule: true,
        consistencyRule: 'Programme plus progressif, détails à vérifier sur la page officielle.',
      ...
      product({
        ...
        description: 'Accès sans évaluation, sans target annoncée, pour traders déjà confiants.',
        ...
        tradableAssets: ['Forex', 'Indices', 'Actions CFD', 'Crypto'],
        hasConsistencyRule: true,
        consistencyRule: "Pas d'évaluation, mais règles de risque et payout à lire ligne par ligne.",
    ...
    auditSummary:
      'Pages officielles FXIFY reliées par programme, page payout ajoutée et page Trustpilot consultée. Le dossier reste partiel tant que Reddit et les preuves de payout indépendantes ne sont pas attachés une par une.',
    auditSourcesChecked: ['Site officiel', 'Pages programmes', 'Page payouts officielle', 'Trustpilot à vérifier'],
    reviewSignals: {
      trustpilotSourceUrl: 'https://www.trustpilot.com/review/fxify.com',
      trustpilotReviewCount: 6055,
      trustpilotFlaggedReviewCount: 3,
      trustpilotFlaggedReviewNote: 'Trustpilot consulté le 2026-07-02 : avis globalement positifs, mais plusieurs avis récents parlent de délais payout ou de refus après accusation de stratégie interdite.',
      payoutIssues: [
        'Trustpilot récent : plainte de payout en attente au-delà du délai annoncé, à vérifier avec la réponse officielle.',
        'Trustpilot récent : refus ou blocage de compte après allégation de stratégie interdite / arbitrage de latence.',
        'Signal mixte : des avis récents rapportent aussi des retraits rapides, donc le point reste à surveiller plutôt qu’à trancher.',
      ],
      confidenceDrivers: ['Pages programmes officielles reliées', 'Page payout officielle reliée', 'Trustpilot relié avec signaux payout mixtes'],
      radarVerdict: 'FXIFY est mieux documentée par programme. Le score reste prudent : Trustpilot est solide en volume, mais les retours payout récents doivent être lus avant achat.',
    },
  }),
  firm({
    id: 12,
  ...
  firm({
    ...
    commercialRelationship: 'Affiliation transparente',
    commercialNote:
      'PropRadar peut recevoir une commission via le lien GOAT Funded Trader. Le code PROPRADAR est affiché comme avantage promotionnel à vérifier au checkout, sans impact sur le score.',
    products: [
      product({
        id: 'goat-challenges',
        name: 'GOAT Challenges / Instant',
        description: 'Programmes 1 step, 2 step, 3 step et Instant avec news trading, weekend holding et plateformes multiples selon configuration.',
        accountSizeMin: 2500,
        accountSizeMax: 400000,
        entryFeeMin: 35,
        profitTarget: 'variable selon modèle',
        maxDailyLoss: '4% à 5% selon modèle',
        maxDrawdown: '6% à 10% selon modèle',
        profitSplit: 'jusqu’à 100% selon conditions',
        platforms: ['MT5', 'TradeLocker', 'cTrader', 'Volumetric'],
        tradableAssets: ['Forex', 'Indices', 'Métaux', 'Crypto', 'Actions/ETF selon plateforme'],
        consistencyRule: 'News trading et weekend holding affichés, mais règles exactes à revérifier au checkout GFT.',
        linkToStart: GOAT_FUNDED_TRADER_AFFILIATE_URL,
      }),
    ],
    strengths: ['News trading affiché', 'Weekend holding affiché', 'Programmes 1 step, 2 step, 3 step et Instant', 'Code PropRadar disponible'],
    weaknesses: ['Forte logique promo', 'Règles variables selon modèle', 'À ne pas choisir uniquement sur le coupon'],
    sources: [
      { label: 'Site officiel GOAT Funded Trader', url: 'https://www.goatfundedtrader.com/' },
      { label: 'Lien inscription PropRadar GFT', url: GOAT_FUNDED_TRADER_AFFILIATE_URL },
    ],
  }),
  firm({
    id: 14,
...
const universePropFirms: PropFirm[] = [
  ...
  universeFirm({
    ...
    productDescription:
      'Programme avec niveaux Jr, Sr et Executive. Le site affiche des comptes de 5k à 1M, overnight/weekend autorisés et payouts bi-weekly selon niveau.',
    accountSizeMin: 5000,
    accountSizeMax: 1000000,
    profitTarget: '5% à 10% selon niveau',
    maxDailyLoss: '3% à 5% selon niveau',
    maxDrawdown: '6% à 10% selon niveau',
    platforms: ['Plateforme propriétaire', 'À vérifier'],
    tradableAssets: ['Forex', 'Crypto', 'Indices', 'Métaux', 'À vérifier'],
    minTradingDays: '3 jours minimum affichés',
    consistencyRule: 'Règles de cohérence et risk review à revérifier dans les conditions officielles avant achat.',
    note:
      "Leveraged est une prop firm active avec comptes de 5k à 1M, split annoncé à 80%, overnight/weekend autorisés sur les tableaux officiels et payouts affichés. À traiter avec prudence : Trustpilot indique une note indisponible pour violation de guidelines et des avis supprimés.",
    additionalSources: [{ label: 'Trustpilot Leveraged', url: 'https://www.trustpilot.com/review/getleveraged.com' }],
    auditStatus: 'Partiellement vérifié',
    auditSummary:
      "Site officiel et Trustpilot consultés : l'offre existe et les règles principales sont visibles, mais le signal Trustpilot est fragilisé par une alerte publique. La fiche doit rester prudente tant que Reddit, Discord, X et preuves de payout indépendantes ne sont pas revus.",
    auditSourcesChecked: ['Site officiel', 'Trustpilot'],
    reviewSignals: {
      redditScore: 55,
      redditMentions: 'volume public à classer manuellement',
      redditSignal: 'Mixte',
      xScore: 54,
      xMentions: 'score brut X/Twitter : marque visible, retours récents à relire avant achat',
      xSignal: 'Mixte',
      trustpilotReliabilityScore: 28,
      trustpilotReliability: 'Faible',
      trustpilotNote:
        "Trustpilot affiche une note indisponible pour violation de guidelines et signale des avis supprimés. Les avis positifs récents existent, mais ils ne doivent pas être utilisés comme preuve forte sans recoupement.",
      trustpilotFlags: [
        'Note Trustpilot indisponible',
        'Alerte publique de violation des guidelines',
        'Avis supprimés signalés par Trustpilot',
        'Avis récents nombreux à recouper avec Discord, X et preuves de payout',
      ],
      trustpilotFlaggedReviewCount: 1,
      trustpilotFlaggedReviewNote:
        'Alerte Trustpilot publique : la note est indisponible et des avis ont été supprimés. Le score de fiabilité reste donc volontairement faible.',
      manipulationRiskScore: 78,
      manipulationRisk: 'Élevé',
      payoutRiskScore: 58,
      payoutRisk: 'Moyen',
      payoutIncidentCount: 1,
      payoutIncidentStatus: 'À surveiller',
      payoutIssues: [
        'Payouts affichés sur le site officiel, mais preuves indépendantes à consolider.',
        'Alerte Trustpilot à prendre en compte avant achat.',
      ],
      radarVerdict:
        "Leveraged mérite d'être dans la base car la marque est visible et active, mais la note Trustpilot indisponible impose une lecture prudente avant achat.",
    },
  }),
];
