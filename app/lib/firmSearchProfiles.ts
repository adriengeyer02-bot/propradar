...
export function getFirmContentLastModified(slug: string, lastReviewed: string) {
  if (!getFirmSearchProfile(slug)) return lastReviewed;
  return FIRM_SEARCH_CONTENT_UPDATED_AT > lastReviewed
    ? FIRM_SEARCH_CONTENT_UPDATED_AT
    : lastReviewed;
}
