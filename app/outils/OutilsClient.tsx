...
function bestReason(firm: PropFirm, style: StyleChoice, priority: PriorityChoice) {
  ...
  if (style === 'EA / Algo' && firm.eaAllowed === 'Oui') return 'EAs allowed, with rules to review before use.';
          ...
          {selectedFirms.map((firm) => (
            <article className="compare-card" key={firm.slug}>
              ...
              <div className="compare-metric-grid">
                ...
                <div><span>Affiliation</span><strong className={`badge ${relationshipClass(firm.commercialRelationship)}`}>{firm.commercialRelationship === 'Affiliation transparente' ? 'Affiliate' : 'No conflict'}</strong></div>
              </div>
