...
const styleNotes: Record<StyleChoice, string> = {
  'SMC / ICT':
    'Priorité aux firms lisibles pour trading manuel : drawdown clair, indices/forex, payout solide et peu de clauses floues.',
  'Swing trading':
    'Recherche des firms plus tolérantes aux positions gardées, avec drawdown Static/EOD et risque payout faible.',
  'Day trading':
    'Profil intraday classique : exécution quotidienne, budget raisonnable, règles claires et score global solide.',
  Scalping:
    'Profil court terme : éviter les firms qui limitent trop les trades rapides, news, EA ou pratiques jugées abusives.',
  'News trading':
    "Profil très sensible : l'outil favorise les firms où la news est autorisée ou moins restrictive.",
  'EA / Algo':
  ...
  'Débutant prudent':
    'Profil sécurité : score fort, payout faible risque, drawdown non trailing et règles simples avant tout.',
};
...
function bestReason(firm: PropFirm, style: StyleChoice, priority: PriorityChoice) {
  ...
  if (style === 'SMC / ICT' && (firm.drawdownType === 'Static' || firm.drawdownType === 'EOD')) return 'Bon fit SMC manuel : drawdown lisible et règles à relire avant exécution.';
  ...
  if (style === 'Scalping') return 'À vérifier sur les clauses de trades rapides, news et stratégies interdites.';
  if (style === 'EA / Algo' && firm.eaAllowed === 'Oui') return 'EA autorisés, avec règles à relire avant usage.';
  if (style === 'News trading' && isNewsAllowed(firm)) return 'News trading plus permissif que la moyenne.';
  if (style === 'Breakout') return 'Profil momentum : surveiller drawdown, spread et règles de consistency.';
  if (priority === 'Prix' && firm.priceFrom > 0) return `Ticket d'entrée autour de ${formatUsd(firm.priceFrom)}.`;
  return firm.bestFor;
}
    ...
    <main className="container tools-page">
      <section className="tools-hero">
        <div>
          ...
          <h1>Compare les prop firms et trouve celle qui colle à ton style.</h1>
          <p className="lead">
            Mets plusieurs firms côte à côte, puis utilise le sélecteur pour trouver une prop adaptée au SMC / ICT,
        ...
        <div className="tools-hero-panel">
          ...
          <div><strong>0</strong><span>score influencé par affiliation</span></div>
        </div>
      </section>
      ...
      <section className="section" id="comparatif">
        <div className="section-heading">
          <div>
            <div className="eyebrow">Comparatif côte à côte</div>
            <h2>Choisis jusqu'à 3 prop firms.</h2>
          </div>
          ...
          {selectedFirms.map((firm) => (
            <article className="compare-card" key={firm.slug}>
              ...
              <div className="compare-metric-grid">
                ...
                <div><span>Affiliation</span><strong className={`badge ${relationshipClass(firm.commercialRelationship)}`}>{firm.commercialRelationship === 'Affiliation transparente' ? 'Affilié' : 'Aucun conflit'}</strong></div>
              </div>
      ...
      <section className="section" id="style-finder">
        <div className="section-heading">
          <div>
            <div className="eyebrow">Sélecteur par style</div>
            <h2>Trouve une prop firm adaptée à ton profil.</h2>
          </div>
          <p className="section-note">Ce score de fit ne remplace pas la fiche : il sert à prioriser ta shortlist.</p>
        </div>
