...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: toEnglishText(guide.title),
...
function guideAnswerParagraphs(guide: NonNullable<ReturnType<typeof getSeoGuideBySlug>>, firms: PropFirm[]) {
  ...
  if (guide.intent === 'promo') {
    return [
      ...
      'The best promo is the one you could almost have bought without a promo: it must fit your style, budget and risk level.',
    ];
  }
  ...
  return [
    ...
    toEnglishText(guide.answer),
    'The right reading order is simple: reliability, rules, total cost, payout, then popularity or promo code.',
  ];
}
...
function guideDecisionCards(guide: NonNullable<ReturnType<typeof getSeoGuideBySlug>>, firms: PropFirm[]) {
  ...
  if (guide.intent === 'comparer' && first && second) {
    return [
      ...
      },
      {
        ...
        text: 'If the difference comes down to a promo, reread payout, drawdown, news, consistency, funded fees and support before paying.',
      },
    ];
  }
  ...
  if (guide.intent === 'promo') {
    return [
      {
        label: 'Real deal',
        text: 'A promo is interesting only if final price, funded-stage fees and rules remain consistent with your budget.',
      },
      {
        label: 'Compare this',
  ...
  return [
    ...
    },
    {
      ...
      text: 'Do not choose only on displayed capital or discount. A bad rule costs more than a promo saves.',
    },
  ];
}
...
export default async function SeoGuidePage({ params }: Props) {
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    headline: toEnglishText(guide.h1),
    description: toEnglishText(guide.metaDescription),
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: toEnglishText(guide.title),
        item: pageUrl,
      },
    ],
  };
    ...
    <main className="container guide-page seo-page">
      ...
      <section className="guide-hero">
        <div>
          ...
          <div className="eyebrow">{toEnglishText(guide.eyebrow)}</div>
          <h1>{toEnglishText(guide.h1)}</h1>
          <p className="lead">{toEnglishText(polishGuideText(guide.answer))}</p>
        ...
        <div className="guide-proof-panel">
          ...
          <div><strong>{guide.primaryKeywords.length}</strong><span>main keywords</span></div>
      ...
      <section className="section guide-card-grid">
        ...
        <article className="panel">
          ...
          <ul className="risk-list">
              {guide.checks.map((check) => <li key={check}>{toEnglishText(polishGuideText(check))}</li>)}
          </ul>
        </article>
      </section>
          ...
          <div className="seo-faq-stack">
            {guide.faq.map((item) => (
              <div key={item.question}>
                <strong>{toEnglishText(item.question)}</strong>
                <p>{toEnglishText(polishGuideText(expandedFaqAnswer(item.answer, guide)))}</p>
              </div>
            ))}
          </div>
        </article>
          ...
          <div className="guide-signal-stack">
            {guide.internalLinks.map((link) => (
              <Link href={link.href} key={link.href}>
                <span>{toEnglishText(link.label)}</span>
                <strong>Open</strong>
              </Link>
            ))}
          </div>
        ...
        <div className="guide-entry-grid">
          {relatedGuides.map((related) => (
            <Link href={`/guides/${related.slug}`} className="guide-entry-card" key={related.slug}>
              <span>{toEnglishText(related.eyebrow)}</span>
              <strong>{toEnglishText(related.title)}</strong>
              <small>{toEnglishText(polishGuideText(related.answer))}</small>
            </Link>
          ))}
        </div>
      </section>
