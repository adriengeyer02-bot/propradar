...
function formatEntryFee(product?: Product) {
  if (!product?.entryFeeMin) return 'À vérifier';
...
function guideAnswerParagraphs(guide: NonNullable<ReturnType<typeof getSeoGuideBySlug>>, firms: PropFirm[]) {
  ...
  if (guide.intent === 'comparer' && first && second) {
    return [
      `La vraie question n'est pas seulement "${first.name} ou ${second.name} ?". Le lecteur veut savoir ce qu'il achète vraiment : une règle de drawdown, un calendrier de payout, une tolérance au style de trading et un niveau de risque opérationnel.`,
      `${first.name} ressort mieux si ton critère principal est la stabilité perçue et la lisibilité du cadre. ${second.name} peut devenir plus intéressant si son programme correspond mieux à ton budget, ton style ou tes contraintes de trading. Le nom de marque ne suffit donc pas : il faut comparer l'offre phare, le coût total et les restrictions.`,
      'Avant de payer, lis la ligne "payout", les conditions de retrait, la règle de perte maximale, la gestion des news et les limites de plateforme. Ce sont ces détails qui transforment une bonne promo en bon choix, ou en piège coûteux.',
    ];
  }

  if (guide.intent === 'style') {
    return [
      `Le lecteur cherche une prop firm compatible avec une manière de trader, pas seulement une note globale. Le bon choix dépend du maintien overnight, des news, des EAs, du drawdown et de la manière dont la firm traite les payouts.`,
      firmNames
        ? `Dans cette recherche, PropRadar compare notamment ${firmNames}. L'objectif est de voir quelle firm colle au style visé sans cacher les restrictions importantes.`
        : 'PropRadar compare les firms qui peuvent correspondre au style visé, puis retire les options trop fragiles ou trop peu documentées.',
      "Le meilleur réflexe est de choisir la règle que tu peux respecter tous les jours, pas la firm qui promet le plus gros capital sur la page marketing.",
    ];
  }

  if (guide.intent === 'risque') {
    return [
      "Le lecteur veut surtout éviter de gagner un challenge puis de se bloquer au moment du retrait. Le payout doit donc être analysé avant le prix, avant le split et avant la taille de compte.",
      'PropRadar regarde les incidents, la clarté des règles, les signaux communautaires et la preuve disponible. Une firm peut avoir une grosse note publique et rester risquée si les conditions de retrait sont floues.',
      'La bonne décision consiste à choisir une firm avec un risque payout lisible, une source officielle claire et des conditions que tu peux vérifier avant achat.',
    ];
  }

  return [
    "Le lecteur veut une réponse exploitable : quelle option regarder en premier, pourquoi elle ressort, et quel détail peut faire changer la décision.",
    guide.answer,
    "La sélection ci-dessous n'est pas un classement publicitaire. Elle croise score PropRadar, risque payout, niveau de preuve, prix minimum, signaux Trustpilot/Reddit et cohérence avec la recherche.",
  ];
}
    ...
    return [
      {
        ...
        text: `${first.name} est à regarder en priorité si tu veux le meilleur compromis score/risque dans cette comparaison.`,
      },
      {
        label: `Choisir ${second.name}`,
        text: `${second.name} devient pertinent si son offre phare colle mieux à ton budget, ton marché ou ton style de trading.`,
      },
      {
  ...
  return [
    {
      ...
      text: 'Commence par éliminer les firms avec risque payout élevé, règles floues ou sources insuffisantes.',
    },
    {
    ...
    },
    {
      ...
      text: "Ne choisis pas uniquement sur le capital affiché ou la réduction. Une mauvaise règle coûte plus cher qu'une promo économise.",
    },
  ];
}
...
function expandedFaqAnswer(answer: string, guide: NonNullable<ReturnType<typeof getSeoGuideBySlug>>) {
  const suffix =
    guide.intent === 'comparer'
      ? " Pour trancher proprement, compare toujours l'offre phare, le risque payout, les restrictions et le coût total avant de regarder la popularité."
      : " Le point important est de vérifier la fiche, les sources officielles et les conditions du programme précis avant de payer.";
