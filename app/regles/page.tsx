...
const RULES_DESCRIPTION =
  'Compare sensitive prop-firm rules covering drawdown, news trading, payouts, consistency, EAs, copy trading and prohibited strategies.';

export const metadata = createPageMetadata({
  title: 'Prop Firm Rules: Drawdown, Payout and Trading Risk',
  description: RULES_DESCRIPTION,
  path: '/regles',
  keywords: ['prop firm rules', 'prop firm drawdown', 'prop firm consistency rule', 'prop firm news trading'],
...
const officialRuleAudits: OfficialRuleAudit[] = [
  {
    ...
    clauses: [
      ...
      {
        title: 'Best Day Rule on 1-Step and FTMO Account',
        severity: 'Élevé',
        finding:
  ...
  },
  {
    ...
    clauses: [
      {
        title: 'Maximum 3% risk and 70% margin cap',
        severity: 'Critique',
        finding:
      ...
      {
        title: 'Quick Strike and trades under 30 seconds',
        severity: 'Élevé',
        finding:
  ...
  },
  {
    ...
    clauses: [
      ...
      {
        title: 'News trading on Performance E8 One',
        severity: 'Élevé',
        finding:
  ...
  },
  {
    ...
    clauses: [
      ...
      {
        title: 'IP and household restrictions',
        severity: 'Élevé',
        finding:
...
const mediumSmallRuleAudits: CompactRuleAudit[] = [
  ...
  },
  {
    ...
    severity: 'Élevé',
    signal:
      'SwiftTrader lists daily loss, maximum loss, maximum allowed risk on open trades and minimum profit per payout.',
    consumerImpact:
...
export default function RulesPage() {
  ...
  return (
    <main className="container rules-page">
      <PageStructuredData
        name="Prop firm rule audits"
        description={RULES_DESCRIPTION}
        path="/regles"
        breadcrumbLabel="Trading rules"
        dateModified="2026-07-13"
        items={officialRuleAudits.map((audit) => ({
          name: getFirmBySlug(audit.slug)?.name ?? audit.slug,
          path: `/firm/${audit.slug}`,
          description: toEnglishText(audit.verdict),
        }))}
      />
      <section className="rules-hero">
        <div>
          ...
          <p className="muted">Page reviewed July 15, 2026. Each audit keeps its own visible source-check date.</p>
        </div>
            ...
            return (
              <article className="rule-audit-card" key={audit.slug}>
                ...
                <div className="rule-audit-source">
                  ...
                  <a href={audit.sourceUrl} target="_blank" rel="noreferrer">{toEnglishText(audit.sourceLabel)}</a>
                </div>
              </article>
            );
          })}
              ...
              <Link href={`/firm/${firm.slug}`} className="longtail-rule-row" key={firm.slug}>
                <div className="firm-result-main">
                  ...
                  <div>
                    ...
                    <span>{toEnglishText(firm.products[0]?.name ?? 'Product to identify')}</span>
                  </div>
                </div>
          ...
          {nextAuditQueue.map((firm) => (
            <Link href={`/firm/${firm.slug}`} className="rules-matrix-row" key={firm.slug}>
              <div className="firm-result-main">
                ...
                <div>
                  <strong>{firm.name}</strong>
                  <span>{toEnglishText(firm.products[0]?.name ?? 'Main product to identify')} - {formatUsd(firm.priceFrom)}</span>
                </div>
              </div>
