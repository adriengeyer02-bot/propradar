...
export const metadata: Metadata = {
  title: 'Règles sensibles des prop firms',
  description:
    'Audit PropRadar des clauses sensibles dans les règles officielles des prop firms : drawdown, news, payout, consistency, EA, copy trading et pratiques interdites.',
};

type RuleSeverity = 'Critique' | 'Élevé' | 'Moyen';
...
function severityClass(severity: RuleSeverity) {
  ...
  if (severity === 'Élevé') return 'badge-amber';
...
function coverageStatusLabel(firm: (typeof propFirms)[number]) {
  if (firm.status === 'Fermée') return 'Archive risque';
  if (firm.auditStatus === 'Vérifié multi-source') return 'Dossier multi-source';
  if (firm.auditStatus === 'Partiellement vérifié') return 'Source repérée';
  return 'À sourcer';
}

function coverageStatusClass(label: string) {
  if (label === 'Dossier multi-source') return 'badge-green';
  if (label === 'Source repérée') return 'badge-blue';
  if (label === 'Archive risque') return 'badge-red';
  return 'badge-neutral';
}
  ...
  return (
    <main className="container rules-page">
      <section className="rules-hero">
        <div>
          <div className="eyebrow">Règles fouillées</div>
          <h1>Les clauses qui peuvent coûter un compte ou un payout.</h1>
          <p className="lead">
            PropRadar lit les pages de règles, help centers et conditions des produits phares pour isoler les clauses dangereuses :
            drawdown, rollover, news, EA, copy trading, consistency, payout caps et sanctions.
          </p>
        </div>
        <div className="rules-hero-panel">
          <div className="summary-stat">
            <span>Audits profonds</span>
            <strong>{officialRuleAudits.length}</strong>
            <small>sources primaires ouvertes</small>
          </div>
          <div className="summary-stat">
            <span>Clauses extraites</span>
            <strong>{clauseCount}</strong>
            <small>{criticalCount} critiques</small>
          </div>
          <div className="summary-stat">
            <span>Firms moyennes</span>
            <strong>{mediumSmallRuleAudits.length}</strong>
            <small>{compactCriticalCount} alertes critiques</small>
          </div>
          <div className="summary-stat">
            <span>Couverture</span>
            <strong>{propFirms.length}</strong>
            <small>firms suivies au total</small>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="section-heading">
          <div>
            <div className="eyebrow">Alertes consommateur</div>
            <h2>Ce que les pages marketing ne disent pas assez fort.</h2>
          </div>
          <Link href="/audit" className="btn">Voir l'audit des sources</Link>
        </div>
                ...
                <div className="rule-audit-top">
                  <div className="firm-result-main">
                    ...
                    <div>
                      ...
                      <div className="firm-subline">{audit.product}</div>
                    </div>
                  </div>
                  {firm ? <span className={`badge ${statusClass(firm.status)}`}>{firm.status}</span> : null}
                </div>
                  ...
                  {audit.clauses.map((clause) => (
                    <div className="clause-item" key={clause.title}>
                      <div className="clause-head">
                        ...
                        <span className={`badge ${severityClass(clause.severity)}`}>{clause.severity}</span>
                      </div>
                      <p>{clause.finding}</p>
                      <div className="impact-box">{clause.consumerImpact}</div>
                    </div>
                  ))}
                </div>

                <div className="rule-audit-source">
                  <span>{audit.sourceDate}</span>
      ...
      <section className="section">
        <div className="section-heading">
          <div>
            <div className="eyebrow">Moyennes & petites firms</div>
            <h2>La longue traîne commence à être lue firm par firm.</h2>
          </div>
          <p className="section-note">
            Ces alertes compactes viennent de sources officielles ou de pages produit repérées. Les dossiers partiels restent volontairement marqués comme tels.
          </p>
        </div>
            ...
            return (
              <article className="compact-rule-card" key={audit.slug}>
                <div className="compact-rule-top">
                  ...
                  <span className={`badge ${severityClass(audit.severity)}`}>{audit.severity}</span>
                </div>

                <div className="compact-rule-tags">
                  <span className={`badge ${proofLevelClass(audit.proofLevel)}`}>{audit.proofLevel}</span>
                  <span>{audit.riskFamily}</span>
                </div>

                <p>{audit.signal}</p>
                <div className="compact-rule-impact">{audit.consumerImpact}</div>
      ...
      <section className="section">
        <div className="section-heading">
          <div>
            <div className="eyebrow">Couverture complète</div>
            <h2>Toutes les autres prop firms restent dans la file d’audit.</h2>
          </div>
          <p className="section-note">
            PropRadar garde les petites firms visibles pour éviter l’angle mort : source repérée, archive risque ou dossier encore à sourcer.
          </p>
        </div>
              ...
              <Link href={`/firm/${firm.slug}`} className="longtail-rule-row" key={firm.slug}>
                <div className="firm-result-main">
                  ...
                  <div>
                    ...
                    <span>{firm.products[0]?.name ?? 'Produit à identifier'}</span>
                  </div>
                </div>
                <span>{firm.drawdownType}</span>
                <span>{firm.newsTrading}</span>
                <span>{firm.eaAllowed}</span>
      ...
      <section className="section">
        <div className="section-heading">
          <div>
            <div className="eyebrow">Fouille suivante</div>
            <h2>Les prochains produits phares à décortiquer.</h2>
          </div>
        </div>
          ...
          {nextAuditQueue.map((firm) => (
            <Link href={`/firm/${firm.slug}`} className="rules-matrix-row" key={firm.slug}>
              <div className="firm-result-main">
                ...
                <div>
                  <strong>{firm.name}</strong>
                  <span>{firm.products[0]?.name ?? 'Produit phare à identifier'} · {formatUsd(firm.priceFrom)}</span>
                </div>
              </div>
      ...
      <section className="section">
        <div className="section-heading">
          <div>
            ...
            <h2>Les firms actives classées par risque payout.</h2>
          </div>
        </div>
